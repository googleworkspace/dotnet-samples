...
{
    // Unit testcase for drive v3 recover drives snippet
    [TestFixture]
    public class RecoverDrivesTest : BaseTest
    {
        ...
        private string CreateOrphanedDrive()
        {
            ...
            var listRequest = service.Permissions.List(driveId);
            listRequest.SupportsAllDrives = true;
            var response = listRequest.Execute();

            foreach (var permission in response.Permissions)
            {
                var deleteRequest = service.Permissions.Delete(driveId, permission.Id);
